...
import { V2Client } from './utils/twitter'
import { Response$sampleStream$Status$200 } from './utils/client-generated'
...
(async () => {
    ...
    try {
        const stream = await (V2Client.sampleStream({
            parameter: {
        ...
        (stream as IncomingMessage).on('data', async (chunk: Buffer) => {
            // Keep alive signal < Buffer 0d 0a > received.Do nothing.
            if (chunk.length > 2) {
                const chunks: Buffer[] = []
                chunks.push(chunk)
                try {
                    const tweet: Response$sampleStream$Status$200["application/json"] = parse(Buffer.concat(chunks).toString())
